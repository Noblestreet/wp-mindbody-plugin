...
function mZ_mindbody_show_schedule( $atts, $account=0 )
{
	...
	if ($type==__('day', 'mz-mindbody-api'))
	{
		...
		$mz_schedule_cache_two = "mz_schedule_week_cache";
	}
	else
	{   
  ...
  // START caching
	...
	$mz_schedule_data_two = $mb->GetClasses($mz_timeframe);	
...
function mz_display_schedule ($events_object, $mb, $grid, $filter, $type, $return, $time_format, $location, $hide, $mz_date, $date_format){
		if(!empty($events_object['GetClassesResult']['Classes']['Class']))
		{
			$mz_days = $mb->makeNumericArray($events_object['GetClassesResult']['Classes']['Class']);
